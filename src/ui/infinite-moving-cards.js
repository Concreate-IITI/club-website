...
export const InfiniteMovingCards = ({ items, direction = "left", speed = "fast", pauseOnHover = true, className }) => {
  ...
  return (
    <div ref={containerRef} className={cn("scroller relative z-20 w-full overflow-hidden", "mask-image:linear-gradient(to right, transparent, white 20%, white 80%, transparent)", className)}>
          ...
          >
            <blockquote>
              <div className="relative z-20 flex flex-col md:flex-row items-start gap-4">
                <img className="w-full md:w-1/2 h-40 md:h-60 object-cover" src={item.image} alt={`${item.name}'s photo`} />
                <div className="flex flex-col">
                  <span className="text-sm md:text-base leading-[1.6] text-white font-normal">{item.quote}</span>
                  <span className="mt-2 flex flex-col">
                    <span className="text-lg md:text-xl font-bold leading-[1.6] text-yellow-200">{item.name}</span>
                    <span className="text-xl md:text-2xl font-bold leading-[1.6] text-rose-500">{item.title}</span>
                  </span>
                </div>
              </div>
            </blockquote>
