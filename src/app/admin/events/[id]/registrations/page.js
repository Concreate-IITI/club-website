  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 text-white p-8">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <Link href="/admin/events" className="inline-flex items-center text-blue-400 hover:text-blue-300 mb-4">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              {event && (
                <>
                  <h2 className="text-2xl text-gray-400 mb-1">{event.title}</h2>
                  <p className="text-gray-500">Total Registrations: {registrations?.length || 0}</p>
                </>
              )}
            </div>

            {registrations && registrations.length > 0 && (
              <button onClick={exportToCSV} className="px-6 py-3 bg-green-600 hover:bg-green-700 rounded-lg font-semibold transition-colors flex items-center">
                <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
